...
@client.query("CREATE OR REPLACE VIEW caramcc_actor_genre_frequency
  AS
  SELECT sg.*, a.*, s.show_id AS sid, MAX(sg.genre)
  FROM #{$actors} a
  JOIN #{$show_actors} sa ON a.actor_id = sa.actor_id
  JOIN #{$tv_shows} s ON sa.show_id = sid
  JOIN #{$show_genres} sg ON sid = sg.show_id")

result = @client.query("SELECT MAX(genre) FROM caramcc_actor_genre_frequency
