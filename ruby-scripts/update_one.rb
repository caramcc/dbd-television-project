...
imdb_or_tvrage = ARGV[0]
@show_id = ARGV[1]
service_id = ARGV[2]


if imdb_or_tvrage == '--imdb'
  r = @client.query("SELECT tvrage_id FROM #{$tv_shows} WHERE show_id = #{@show_id} LIMIT 1")
  tvrage_id = ''
  r.each do |row|
    tvrage_id = row['tvrage_id']
  end
  puts "updating imdb for tvr #{tvrage_id}"
  imdb_id = service_id
  update_record(update_imdb(imdb_id, update_tvr(tvrage_id)))
elsif imdb_or_tvrage == '--tvr'
  r = @client.query("SELECT imdb_id FROM #{$tv_shows} WHERE show_id = #{@show_id} LIMIT 1")
  imdb_id = ''
  r.each do |row|
    imdb_id = row['imdb_id']
  end
  tvrage_id = service_id
  update_record(update_imdb(imdb_id, update_tvr(tvrage_id)))
  puts "updating tvr for imdb: #{imdb_id}"
end
